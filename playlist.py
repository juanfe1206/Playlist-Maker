...
#complex functions:
def name_playlist(driver, text_field_id, save):
  try:
    element = WebDriverWait(driver, 12).until(
...
def create_playlist(driver, my_playlist_name):
  ...
  time.sleep(0.6)
  name_playlist(driver, 'playlist-edit-details-name-input', save=my_playlist_name)
  ...
  completion = client.chat.completions.create(
      ...
      messages=[
          {"role": "system", 
           ...
           Then, with this information your task is to only return a list of songs of size 15 and no additional text as follow:
           'song - Artist, Song - Artist, song - Artist'
           If no artists are given as reference, it is up to you to choose artists that fit the mood, it is specially good in this case to use popular artists.
...
list_of_songs, my_playlist_name = create_list_of_songs('happy pop songs', artists='Taylor Swift, Billie Eilish')
...
login(driver)
time.sleep(1)
create_playlist(driver, my_playlist_name)
time.sleep(0.5)
